...
class AgentOrchestrationIntegrationTest {
    ...
    void singleTurnCompletion_noToolCalls_subprocessCompletes() {
        ...
        when(llmService.call(any(), any(), any(), anyList()))
        ...
        verify(llmService, times(1)).call(any(), any(), any(), anyList());
    }
    ...
    void singleToolCall_singleTurn_completesAfterToolResult() {
        ...
        Set<String> pending = stateManager.loadPendingToolCalls(adHocExec.getId());
        assertThat(pending).containsExactly("call-1");
        ...
        verify(llmService, times(2)).call(any(), any(), any(), anyList());
        assertProcessEnded(pi);
    }
    ...
    void multiTurn_twoSequentialToolCalls_completesAfterBoth() {
        ...
        verify(llmService, times(3)).call(any(), any(), any(), anyList());
        assertProcessEnded(pi);
    }
    ...
    void parallelToolCalls_allCompleteBeforeNextLlmCall() {
        ...
        when(llmService.call(any(), any(), any(), anyList()))
                .thenReturn(parallelToolCallResponse(
                        new ToolCallRequest("call-1", "toolA"),
        ...
        assertThat(adHocExec).isNotNull();
        Set<String> pending = stateManager.loadPendingToolCalls(adHocExec.getId());
        assertThat(pending).containsExactlyInAnyOrder("call-1", "call-2");
    ...
    void inactiveExecution_orchestrationStepSkipped() {
        ...
        List<Job> remainingJobs = managementService.createJobQuery()
    ...
    private void assertProcessEnded(ProcessInstance pi) {
        assertThat(runtimeService.createProcessInstanceQuery()
                ...
                .as("Process instance should have ended")
                .isNull();
    }
    ...
    private void assertProcessNotEnded(ProcessInstance pi) {
        assertThat(runtimeService.createProcessInstanceQuery()
                .processInstanceId(pi.getId()).singleResult())
                .as("Process instance should still be running")
                .isNotNull();
    }
    ...
    private static LlmResponse textOnlyResponse(String text) {
        return new LlmResponse(text, List.of(), List.of(
                ConversationEntry.assistant(text, List.of())));
    }
    ...
    private static LlmResponse toolCallResponse(String toolCallId, String toolId) {
        ...
        return new LlmResponse(null, toolCalls, List.of(
                ConversationEntry.assistant(null, toolCalls)));
    }
    ...
    private void deploy(String bpmnXml) {
        deploymentId = repositoryService.createDeployment()
                .addString("agent-process.bpmn", bpmnXml)
                .deploy()
                .getId();
    }
        ...
        for (String toolId : toolActivityIds) {
            tools.append("""
                  <serviceTask id="%s" name="%s" camunda:class="%s">
                    <extensionElements>
                      <camunda:inputOutput>
                        <camunda:outputParameter name="toolOutput">${toolOutput}</camunda:outputParameter>
                      </camunda:inputOutput>
                    </extensionElements>
                  </serviceTask>
